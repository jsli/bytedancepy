...
from bytedancepy.constants import ByteDanceErrorCode
from bytedancepy.exceptions import ByteDanceClientException, APILimitedException
from bytedancepy.session.memorystorage import MemoryStorage
...
def _is_api_endpoint(obj):
    return isinstance(obj, BaseByteDanceAPI)


class BaseByteDanceClient(object):
    API_BASE_URL = ''

    def __new__(cls, *args, **kwargs):
        self = super(BaseByteDanceClient, cls).__new__(cls)
    ...
    def _request(self, method, url_or_endpoint, **kwargs):
        ...
        try:
            ...
            raise ByteDanceClientException(
                errcode=None,
                errmsg=None,
                client=self,
    ...
    def _handle_result(self, res, method=None, url=None,
        ...
        if not isinstance(res, dict):
            # Dirty hack around asyncio based AsyncByteDanceClient
        ...
        if 'errcode' in result and result['errcode'] != 0:
            ...
            if self.auto_retry and errcode in (
                    ByteDanceErrorCode.INVALID_CREDENTIAL.value,
                    ByteDanceErrorCode.INVALID_ACCESS_TOKEN.value,
                    ByteDanceErrorCode.EXPIRED_ACCESS_TOKEN.value):
            ...
            elif errcode == ByteDanceErrorCode.OUT_OF_API_FREQ_LIMIT.value:
    ...
    def _get(self, url, **kwargs):
        warnings.warn('`_get` method of `ByteDanceClient` is deprecated, will be removed in 1.6,'
                      'Use `get` instead',
    ...
    def _post(self, url, **kwargs):
        warnings.warn('`_post` method of `ByteDanceClient` is deprecated, will be removed in 1.6,'
                      'Use `post` instead',
                      DeprecationWarning, stacklevel=2)
